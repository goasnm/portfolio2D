...
void AstarScene::Update()
{
	...
	{
		SceneManager::GetSingleton()->ChangeScene("타일맵툴", "로딩");
		return;
	}
...
AstarTile* AstarScene::GetMinTotalCostTileWithHeap()
{
	...
	// 자식과 비교하면서 정렬한다. ( 이진트리 -> 재귀함수를 쓴다와 같은 의미)
	if (heap.empty() == false)
	{
	UpdateLower(heap[0]);
	}
...
void AstarScene::UpdateLower(AstarTile* tile)	// 인덱스 : 3 이면
{
	int lChildIndex = tile->GetHeapIndex() * 2 + 1;

	if (lChildIndex >= heap.size())	return;

	AstarTile* lChild = heap[lChildIndex];	//부모의 인덱스 * 2 (6)

	AstarTile* minChild = lChild;
	int rChildIndex = tile->GetHeapIndex() * 2 + 2;
	if (rChildIndex < heap.size())
	{
		AstarTile* rChild = heap[rChildIndex];	//부모의 인덱스 * 2 + 1 (7)
		// 비교
