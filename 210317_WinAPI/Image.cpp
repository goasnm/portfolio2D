    ...
    bool isTransparent/* = FALSE*/, COLORREF transColor/* = FALSE*/)
{
    ...
    imageInfo->hTempBit = CreateCompatibleBitmap(hdc,
        imageInfo->width, imageInfo->height);
    imageInfo->hTempBit = (HBITMAP)SelectObject(imageInfo->hTempDC,
        imageInfo->hTempBit);
...
void Image::MapRender(HDC hdc, int destX, int destY, bool isCenterRenderring, float sizeX, float sizeY)
{
    int x = destX;
    int y = destY;
    if (isCenterRenderring)
    ...
    if (isTransparent)
    {
        PatBlt(imageInfo->hTempDC, 0, 0, WINSIZE_X, WINSIZE_Y , BLACKNESS);
